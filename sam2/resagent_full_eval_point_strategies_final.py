...
def load_qwen_model(model_path: str, device: torch.device):
    ...
    if Qwen2_5_VLForConditionalGeneration is not None:
        try:
            model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                model_path,
                torch_dtype=torch.bfloat16 if device.type == "cuda" else torch.float16 if device.type == "mps" else torch.float32,
    ...
    if model is None:
        raise RuntimeError(f"Failed to load VLM from {model_path}. Last error: {last_err}")
...
def render_multi_marker_query(
    ...
    pad = int(round(effective_marker_size * pad_scale))
    crop_x1 = int(max(0, min([x1] + [p[0] - effective_marker_size - pad for p in points])))
    crop_y1 = int(max(0, min([y1] + [p[1] - effective_marker_size - pad for p in points])))
    crop_x2 = int(min(img.size[0], max([x2] + [p[0] + effective_marker_size + pad for p in points])))
    crop_y2 = int(min(img.size[1], max([y2] + [p[1] + effective_marker_size + pad for p in points])))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--validation_mode", type=str, default="multi_marker", choices=["multi_marker"])
...
def resolve_model_path(model_root: str, model_name: str) -> str:
    ...
    return os.path.join(model_root, mapping.get(model_name, model_name))
