...
Reproducibility:
- global seed controls per-ref RNG via seed + ref_id * 1000003
...
def perturb_box_all_directions(
    ...
    """Deterministic bbox perturbation in all directions (expand/shrink)."""
...
def compute_iou(pred_mask: np.ndarray, gt_mask: np.ndarray) -> float:
    """pred_mask/gt_mask: {0,1} uint8/bool arrays [H,W]"""
...
def run_sam_single_mask(
    ...
    """
    Run SAM2 predictor with multimask_output=False and return a binarized mask [H,W] uint8 {0,1}.
    """
    ...
    # masks might be (1,H,W) or (H,W)
    if masks.ndim == 3:
        m = masks[0]
    else:
        m = masks
    # SAM2 commonly returns boolean masks already; still keep threshold for safety
...
# -----------------------------
def parse_args():
    parser = argparse.ArgumentParser("Build SAM2 ΔIoU teacher dataset for learned EBD.")
    ...
    parser.add_argument("--out_dir", type=str, default="./sam2_teacher_npz_iou",
...
def main():
    ...
    # ---- out dir ----
    tag = f"{args.task}_{args.split}_seed{args.seed}_N{args.num_points}_pert{args.bbox_min_ratio}-{args.bbox_max_ratio}_{args.sam2_variant}_dIoU"
    ...
    for ref_id in tqdm(ref_ids):
        ...
        W, H = img.size
        gt_mask = refer.getMask(refs)["mask"].astype(np.uint8)  # [H,W], 0/1
        ...
        for i, (x, y) in enumerate(pts):
            m_pos = run_sam_single_mask(
                predictor, box_xyxy,
                point_xy=(x, y), point_label=1,
                mask_threshold=args.mask_threshold
            )
            m_neg = run_sam_single_mask(
                predictor, box_xyxy,
                point_xy=(x, y), point_label=0,
                mask_threshold=args.mask_threshold
            )
