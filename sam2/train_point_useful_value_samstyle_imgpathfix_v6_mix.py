...
def _infer_tag_from_dir(npz_dir: str) -> str:
    s = npz_dir.rstrip('/').split('/')[-1]
...
class MultiTeacherNPZDataset(Dataset):
    """Merge multiple NPZ dirs into one dataset with a per-sample `dataset` tag.

    This is useful for mixed training (RefCOCO / RefCOCO+ / RefCOCOg) and for
    balanced sampling.

    Each returned sample dict additionally contains:
        - dataset: a short tag string
        - _npz_path: path to the npz file (for debugging)

    It keeps the same key-normalization behavior as TeacherNPZDataset:
        - img_path -> image_path
        - re-resolve image_path by basename/file_name under --data_root when needed
    """

    def __init__(self, npz_dirs, dataset_tags=None, data_root: str = ""):
        if isinstance(npz_dirs, str):
            npz_dirs = [npz_dirs]
        self.npz_dirs = list(npz_dirs)
        if dataset_tags is None or len(dataset_tags) == 0:
            dataset_tags = [_infer_tag_from_dir(d) for d in self.npz_dirs]
        if len(dataset_tags) != len(self.npz_dirs):
            raise ValueError(f"dataset_tags length {len(dataset_tags)} must match npz_dirs length {len(self.npz_dirs)}")
        self.dataset_tags = list(dataset_tags)
        self.data_root = data_root

        self.paths = []
        self.tags_per_item = []
        for d, tag in zip(self.npz_dirs, self.dataset_tags):
            files = sorted(glob.glob(os.path.join(d, "*.npz")))
            if len(files) == 0:
                raise FileNotFoundError(f"No .npz found under: {d}")
            self.paths.extend(files)
            self.tags_per_item.extend([tag] * len(files))

        # basic stats
        from collections import Counter
        self.ds_counts = Counter(self.tags_per_item)
...
def collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    datasets = [_to_py_str(b.get('dataset', b.get('ds', ''))) for b in batch]
...
def parse_args() -> argparse.Namespace:
    ...
    # dataset dirs
    p.add_argument("--train_npz_dir", type=str, default="", help="(legacy) single train npz dir")
    p.add_argument("--train_npz_dirs", type=str, nargs="*", default=None,
                   help="One or more train NPZ dirs (overrides --train_npz_dir).")
    p.add_argument("--train_tags", type=str, nargs="*", default=None,
                   help="Optional tags for each train_npz_dirs entry (e.g., refcoco refcoco+ refcocog).")

    p.add_argument("--val_npz_dir", type=str, default="", help="(legacy) single val npz dir")
    p.add_argument("--val_npz_dirs", type=str, nargs="*", default=None,
                   help="One or more val NPZ dirs (overrides --val_npz_dir).")
    p.add_argument("--val_tags", type=str, nargs="*", default=None,
                   help="Optional tags for each val_npz_dirs entry.")
    p.add_argument("--val_ratio", type=float, default=0.02)
    p.add_argument("--data_root", type=str, default="",
                   help="RefCOCO(-/+ /g) dataset root (used to re-resolve image paths when NPZ paths are not valid).")
    ...
    # mixed training (multiple datasets)
    p.add_argument("--mix_strategy", type=str, default="proportional",
                   choices=["proportional", "uniform", "custom"],
                   help="How to mix multiple train datasets: proportional (by size), uniform (each dataset ~equal), custom (use --mix_weights).")
    p.add_argument("--mix_weights", type=float, nargs="*", default=None,
                   help="Dataset-level weights for custom mixing (same length as train_npz_dirs).")
    p.add_argument("--mix_epoch_size", type=int, default=0,
                   help="If >0 and using uniform/custom mixing, number of samples drawn per epoch (with replacement). Default uses full train set size.")
...
def main() -> None:
    ...
    # datasets (support multi-dataset mixing)
    def _resolve_dirs_and_tags(legacy_dir, dirs_list, tags_list, which: str):
        # legacy_dir: --*_npz_dir, dirs_list: --*_npz_dirs
        if dirs_list is not None and len(dirs_list) > 0:
            dirs = list(dirs_list)
        else:
            dirs = [legacy_dir] if legacy_dir else []
        dirs = [d for d in dirs if d]
        if which == "train" and len(dirs) == 0:
            raise ValueError("No train NPZ dir(s) provided. Use --train_npz_dir or --train_npz_dirs.")
        if tags_list is not None and len(tags_list) > 0:
            if len(tags_list) != len(dirs):
                raise ValueError(f"--{which}_tags must have same length as --{which}_npz_dirs.")
            tags = list(tags_list)
        else:
            tags = [_infer_tag_from_dir(d) for d in dirs]
        return dirs, tags

    train_dirs, train_tags = _resolve_dirs_and_tags(args.train_npz_dir, args.train_npz_dirs, args.train_tags, which="train")
    val_dirs = []
    val_tags = []
    if (args.val_npz_dirs is not None and len(args.val_npz_dirs) > 0) or (args.val_npz_dir and os.path.isdir(args.val_npz_dir)):
        val_dirs, val_tags = _resolve_dirs_and_tags(args.val_npz_dir, args.val_npz_dirs, args.val_tags, which="val")

    train_full = MultiTeacherNPZDataset(train_dirs, train_tags, data_root=args.data_root)

    if len(val_dirs) > 0:
        train_ds = train_full
        val_ds = MultiTeacherNPZDataset(val_dirs, val_tags, data_root=args.data_root)
        log("Dataset split: explicit val_npz_dir(s) used.")
        log(f"train_npz_dirs = {train_dirs}  (#files={len(train_full)})")
        log(f"  train by dataset: {train_full.ds_counts}")
        log(f"val_npz_dirs   = {val_dirs}  (#files={len(val_ds)})")
        log(f"  val by dataset:   {val_ds.ds_counts}")
    else:
    ...
    # mixed sampling (optional)
    train_sampler = None
    train_shuffle = True
    if args.mix_strategy != "proportional":
        # collect per-sample tags for the active train_ds (handles Subset)
        if isinstance(train_ds, torch.utils.data.Subset):
            base = train_ds.dataset
            idxs = train_ds.indices
            tags = [base.tags_per_item[i] for i in idxs]
            base_train_tags = getattr(base, "dataset_tags", train_tags)
        else:
            tags = getattr(train_ds, "tags_per_item", ["train"] * len(train_ds))
            base_train_tags = getattr(train_ds, "dataset_tags", train_tags)
        ...
        if len(counts) <= 1:
            ...
            if args.mix_strategy == "uniform":
                ds_w = {t: 1.0 for t in counts.keys()}
            else:
                if args.mix_weights is None or len(args.mix_weights) == 0:
                    raise ValueError("mix_strategy=custom requires --mix_weights")
                if len(args.mix_weights) != len(base_train_tags):
                    raise ValueError("--mix_weights length must match number of train datasets (train_tags).")
                ds_w = dict(zip(base_train_tags, list(args.mix_weights)))

            weights = [float(ds_w.get(t, 1.0)) / float(counts[t]) for t in tags]
            num_samples = int(args.mix_epoch_size) if args.mix_epoch_size > 0 else len(weights)
            train_sampler = WeightedRandomSampler(weights, num_samples=num_samples, replacement=True)
            train_shuffle = False
            log(f"Mix sampling enabled: strategy={args.mix_strategy} epoch_size={num_samples} (replacement=True)")
            log(f"  active-train counts: {dict(counts)}")
            if args.mix_strategy == "custom":
                log(f"  dataset weights: {ds_w}")
